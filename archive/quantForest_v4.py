...
class QuantileForest():
    ...
    def computeQuantile(self, X, alpha, doOptim=True, verbose=False,
        ...
        for k in range(numRegressor):  # For each regressor
            ...
            weightPerTree = (self._nodesOfSamples == nodesOfRegressor[:, k])*1.
            ...
            if doOptim:  # Computation is by optimisation
                ...
                y0 = np.percentile(self._outputSample[weight != 0], alpha*100)
    ...
    def _compute(self, X, alpha):
        """

        """
        verbose = False
        # Nodes of the regressor in all the trees
        nodesOfRegressor = self._forest.apply(X)

        # Set to 1 only the samples in the same nodes the regressor,
        # Shape : Matrix (numSample * numTree)
        weightPerTree = (self._nodesOfSamples == nodesOfRegressor)*1.
        ...
        if self._optMethod == "Cobyla":
            quantiles = fmin_cobyla(self._optFunc,
                                    y0,
                                    [self._ieqFunc],
                                    args=(weight, alpha),
                                    disp=verbose)
            ...
            epsilon = 1.E-1 * abs(y0[i])
            quantiles = fmin_slsqp(self._optFunc,
                                   y0,
                                   f_ieqcons=self._ieqFunc,
                                   args=(weight, alpha),
                                   disp=verbose,
    ...
    def findMinQuantile(self, alpha):
        """
...
#        es.optimize(self.computeQuantile, args=(alpha))
        ...
        ret = basinhopping(self._compute, x0, minimizer_kwargs=minimizer_kwargs)
        return ret
...
# ==============================================================================
    ...
    def computeForest(self,
        ...
        oobID = [-trees[i].indices_ for i in range(n_estimators)]
        samplesOOB = pd.DataFrame(oobID).T
    ...
    def setPrecisionOfCDF(self, n_points):
        ...
        if n_points == 0:  # We use the outputSample as precision vector
            self._yCDF = pd.Series(np.sort(self._outputSample))
...
#    numSample = 10
#    dim = 2
#    X = np.ones((numSample, dim))
#    y = np.ones((numSample,))
#
#    quantReg = QuantileForest(X, y)
#    alphai = 0.1
#    rhoi = [0.5]*dim
#    quant = quantReg.computeQuantile(rhoi, alphai)
