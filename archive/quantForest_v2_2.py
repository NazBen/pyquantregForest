...
import numpy as np
from scipy.optimize import basinhopping, fmin, fmin_powell, fmin_bfgs
...
class QuantileForest(RandomForestRegressor):
    """Quantile Regresion Random Forest.
      ...
      max_leaf_nodes : int or None,
                      optional (default=max(10, len(outputSample)/100))
      ...
      outputSample : string, optional (default="Simplex")
        Name of the Optimisation method to find the alpha-quantile (if the
        option is chosen in the predict method).
    ...
    def __init__(self,
        ...
        self.fit(n_estimators, min_samples_leaf, n_jobs, random_state,
                 *more_args, **more_kwargs)
    ...
    def predict(self, X, alpha, doOptim=True, verbose=False, doSaveCDF=False,
                iTree=-1):
        """
        ...
        if iTree < 0:
            nodesOfRegressor = self.apply(X).transpose()
        ...
        for k in range(numRegressor):  # For each regressor
            ...
            if doOptim:  # Computation is by optimisation
                ...
                for i, alphai in enumerate(alpha):
                    if self._optMethod == "Simplex":
                        optFunc = fmin
                    elif self._optMethod == "Powell":
                        optFunc = fmin_powell
                    elif self._optMethod == "BFGS":
                        optFunc = fmin_bfgs
                    else:
                        raise Exception("Unknow optimisation method %s" %
                                        self._optMethod)
                    quantiles[k, i] = optFunc(self._checkFunction,
                                              y0[i],
                                              args=(weight, alphai),
                                              disp=verbose)
            else:
    ...
    def _checkFunction(self, yi, w, alpha):
        """

        """
        posVal = np.where(w > 0.)[0]  # Positive weights
        u = self._outputSample.values[posVal] - yi
        y = w[posVal].dot(u * (alpha - (u < 0.)*1.))
        return y
...
# =============================================================================
    ...
    def setInputSample(self, inputSample):
        ...
        error = ValueError("Don't build a Forest with only one observation...")
    ...
    def fit(self,
        ...
        super(QuantileForest, self).__init__(n_jobs=n_jobs,
        ...
        super(QuantileForest, self).fit(self._inputSample, self._outputSample)

        # Nodes of each sample in all the tree
        self._nodesOfSamples = DataFrame(self.apply(self._inputSample))
#        print self._nodesOfSamples.memory_usage().sum()

        trees = self.estimators_  # List of constructed trees
