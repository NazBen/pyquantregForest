...
class QuantileForest() :
  ...
  def __init__(self, inputSample,
    ...
    self._forest = forest.fit(inputSample, outputSample.ravel())
    ...
    samplesOOB = np.array([-self._forest.estimators_[i].indices_ for i in range(self._numTree)]).transpose()
    self._nodesOfSamples[samplesOOB] = -1
    ...
    if numPoints == 0 :
      self._yy = np.sort(outputSample.ravel())
    else :
      yymin = min(self._outputSample)
      yymax = max(self._outputSample)
      self._yy = np.linspace(yymin, yymax, numPoints)

    # Matix of output samples inferior to a quantile value
    self._infYY = np.matrix(( self._outputSample <= self._yy ).transpose())
  ...
  def computeQuantile(self, X, alpha, doOptim=False) :
    ...
    for k in range(numRegressor) : # For each regressor
      ...
      weight.resize(self._numSample, 1) # Resize in a casual size

      if doOptim :
        y0 = np.mean(self._outputSample[weight != 0])
        ...
        CDF = np.array(self._infYY*weight).ravel() # Compute the CDF
        quantiles[k, :] = [ self._yy[CDF >= alphai][0] for alphai in alpha]

    return quantiles

  def _optFunc(self, yi, w, alpha) :
    alphai = w[self._outputSample <= yi].sum()
...
#  def createForest(self, inputSample, outputSample) :
#    self._inputSample = inputSample
#    self._outputSample = outputSample
#    self._numSample = len(outputSample)
#    self.buildForest()
#
...
#  def buildForest(self, n_jobs=n_jobs,
#                  n_estimators=n_estimators,
#                  min_samples_leaf=min_samples_leaf,
#                  random_state=random_state) :
#
...
#                                   random_state=random_state)
#
#    self._forest = forest.fit(self._inputSample, self._outputSample.ravel())